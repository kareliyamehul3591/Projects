...
module.exports = {
  userlist: (req, res) => {
    // res.redirect('/login');
    userlist("end_user", (error, result) => {
      if (!error) {
        res.locals = { title: 'User List' };
        res.render('admin/user/user_list', { 'user_data': result, auth_data: req.session.auth_data, message: req.flash('message'), status: req.flash('status') });
      }
    });

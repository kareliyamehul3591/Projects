...
use App\Models\App_test_question;
...
class AppTestController extends Controller
{
    ...
    {
        return view('app_test.list');
    }

    public function appTestList(Request $request)
    {
        $q = $request->input('q');
        ...
        $values = App_test::join('packages', 'app_tests.package_id', '=', 'packages.id')
            ->leftJoin('institutes', 'app_tests.institute_id', '=', 'institutes.id');
        ...
        if ($q && $opts) {
            foreach ($opts as $key => $opt) {
                if ($opt == "package_list") {
                    ...
                    $values = $values->where('app_tests.' . $opt, 'LIKE', "%$q[$key]%");
                }
            }
        }
        ...
        return response()->json([
            ...
            "Records" => $values->offset($start)->limit($limit)->get([
                ...
                $recode->show = route('app_test.show', [$recode->id]);
                $recode->edit = route('app_test.edit', [$recode->id]);
                $recode->delete = route('app_test.delete', [$recode->id]);
                ...
                $recode->total_student = Student_app_test::where('app_test_id',$recode->id)->count();
     ...
     */
    public function show(App_test $app_test)
    {
        return view('app_test.show', compact('app_test'));
    }

    public function appQuestionPaperTestList(Request $request, $id)
    {
        $q = $request->input('q');
        $opts = $request->input('opt');
        $start = (($request->input('jtStartIndex')) ? $request->input('jtStartIndex') : 0);
        ...
        if ($q && $opts) {
            foreach ($opts as $key => $opt) {
                ...
                } else if ($opt == "app_test_id") {
                    $test = App_test::find($q[$key]);
    ...
    public function create()
    {
        ...
        return view('app_test.create', compact('institutes', 'packages', 'subject_grades'));
    }

    /**
     * Store a newly created record.
     */
    public function store(Request $request, App_test $app_test)
    {
        ...
        $app_test = App_test::create($data);

        foreach (explode(',', $data['questions']) as $questions) {
            $app_test->questions()->create([
        ...
        $templateData = [
            'name' => $app_test->name,
            'test_name' => $app_test->name,
        ];

        $student_packages = Student_package::whereIn('package_id', explode(',', $app_test->package_id))->groupBy('user_id')->pluck('user_id')->toArray();
        ...
        foreach ($users as $user) {
            $stestData = [
                ...
                'test_name' => $app_test->name,
                'valid_from' => $app_test->available_from,
                'valid_to' => $app_test->available_to,
            ];
        ...
        return redirect()->route('app_test.index')->with('success', 'App Test created successfully.');
    }

    /**
     * Show the form for editing a record.
     */
    public function edit(App_test $app_test)
    {
        $institutes = Institutes::where('status', 1)->get();
        $packages = Package::where('status', 1)->get();
        $subject_grades = Subject_grade::where('status', 1)->get();
        return view('app_test.edit', compact('app_test', 'institutes', 'packages', 'subject_grades'));
    }

    /**
     * Update a created record.
     */
    public function update(Request $request, App_test $app_test)
    {
        $data = $request->validate([
            'name' => 'required',
            'description' => 'nullable',
        ...
        $app_test->update($data);

        $questions_id = [];
        foreach (explode(',', $data['questions']) as $questions) {
            $questions = $app_test->questions()->create([
                'question_id' => $questions,
            ]);
            $questions_id[] = $questions->id;
        }
        $app_test->questions()->whereNotIn('id', $questions_id)->delete();

        return redirect()->route('app_test.index')->with('success', 'App Test updated successfully.');
    }

    /**
     * Remove the specified record from storage.
     */
    public function destroy(App_test $app_test)
    {
        $app_test->delete();

        return redirect()->route('app_test.index')->with('success', 'App Test deleted successfully');
    }

    /**
     * Remove the multiple records from storage.
     */
    public function appTestMultipleDelete(Request $request)
    {
        $ids = $request->input('ids');

        foreach ($ids as $id) {
            App_test::where('id', $id)->delete();
        }
    ...
    {
        $id = $request->id;
        $app = App_test::find($request->ATid);
        $subject_grades = Subject_grade::find($id);
        $language = $request->input('language');
        return view('app_test.questionsBy', compact('subject_grades', 'app', 'id', 'language'));
    }
    ...
    public function questionList(Request $request)
    {
        ...
        if ($q && $opts) {
            foreach ($opts as $key => $opt) {
                if ($opt == "app_id") {
                    $question_id = App_test_question::where('app_test_id', $q[$key])->pluck('question_id')->toArray();

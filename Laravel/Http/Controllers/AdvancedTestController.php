...
use App\Models\Advanced_test_question;
...
class AdvancedTestController extends Controller
{
    ...
    {
        return view('advanced_test.list');
    }

    public function advancedTestList(Request $request)
    {
        $q = $request->input('q');
        ...
        $values = Advanced_test::join('packages', 'advanced_tests.package_id', '=', 'packages.id')
            ->leftJoin('institutes', 'advanced_tests.institute_id', '=', 'institutes.id');
        ...
        if ($q && $opts) {
            foreach ($opts as $key => $opt) {
                if ($opt == "package_list") {
                    ...
                    $values = $values->where('advanced_tests.' . $opt, 'LIKE', "%$q[$key]%");
                }
            }
        }
        ...
        return response()->json([
            ...
            "Records" => $values->offset($start)->limit($limit)->get([
                ...
                $recode->show = route('advanced_test.show', [$recode->id]);
                $recode->edit = route('advanced_test.edit', [$recode->id]);
                $recode->delete = route('advanced_test.delete', [$recode->id]);
                ...
                $recode->total_student = Student_advanced_test::where('advanced_test_id',$recode->id)->count();
     ...
     */
    public function show(Advanced_test $advanced_test)
    {
        return view('advanced_test.show', compact('advanced_test'));
    }

    public function advancedQuestionPaperTestList(Request $request, $id)
    {
        $q = $request->input('q');
        $opts = $request->input('opt');
        $start = (($request->input('jtStartIndex')) ? $request->input('jtStartIndex') : 0);
        ...
        if ($q && $opts) {
            foreach ($opts as $key => $opt) {
                ...
                } else if ($opt == "advanced_test_id") {
                    $test = Advanced_test::find($q[$key]);
    ...
    public function create()
    {
        ...
        return view('advanced_test.create', compact('institutes', 'packages', 'reference_exams'));
    }

    /**
     * Store a newly created record.
     */
    public function store(Request $request, Advanced_test $advanced_test)
    {
        ...
        $advanced_test = Advanced_test::create($data);

        foreach (explode(',', $data['questions']) as $questions) {
            $advanced_test->questions()->create([
                'question_id' => $questions,
            ]);
        }

        $templateData = [
            'name' => $advanced_test->name,
            'test_name' => $advanced_test->name,
        ];

        $student_packages = Student_package::whereIn('package_id', explode(',', $advanced_test->package_id))->groupBy('user_id')->pluck('user_id')->toArray();
        ...
        foreach ($users as $user) {
            $stestData = [
                ...
                'test_name' => $advanced_test->name,
                'valid_from' => $advanced_test->available_from,
                'valid_to' => $advanced_test->available_to,
            ];
        ...
        return redirect()->route('advanced_test.index')->with('success', 'Advanced Test created successfully.');
    }

    /**
     * Show the form for editing a record.
     */
    public function edit(Advanced_test $advanced_test)
    {
        $institutes = Institutes::where('status', 1)->get();
        $packages = Package::where('status', 1)->get();
        $reference_exams = Reference_exam::where('status', 1)->get();
        return view('advanced_test.edit', compact('advanced_test', 'institutes', 'packages', 'reference_exams'));
    }

    /**
     * Update a created record.
     */
    public function update(Request $request, Advanced_test $advanced_test)
    {
        $data = $request->validate([
            'name' => 'required',
            'description' => 'nullable',
        ...
        $advanced_test->update($data);

        $questions_id = [];
        foreach (explode(',', $data['questions']) as $questions) {
            $questions = $advanced_test->questions()->create([
                'question_id' => $questions,
            ]);
            $questions_id[] = $questions->id;
        }
        $advanced_test->questions()->whereNotIn('id', $questions_id)->delete();

        return redirect()->route('advanced_test.index')->with('success', 'Advanced Test updated successfully.');
    }

    /**
     * Remove the specified record from storage.
     */
    public function destroy(Advanced_test $advanced_test)
    {
        $advanced_test->delete();

        return redirect()->route('advanced_test.index')->with('success', 'Advanced Test deleted successfully');
    }

    /**
     * Remove the multiple records from storage.
     */
    public function advancedTestMultipleDelete(Request $request)
    {
        $ids = $request->input('ids');

        foreach ($ids as $id) {
            Advanced_test::where('id', $id)->delete();
        }
    ...
    public function questionsBy(Request $request)
    {
        ...
        $id = $request->id;
        $data = [];
        $advanced = Advanced_test::find($request->BQid);
        ...
        return view('advanced_test.questionsBy', compact('type', 'id', 'data', 'advanced'));
    }
    ...
    public function questionList(Request $request)
    {
        ...
        if($opts[0] == 'question_description')
        {
            $values = Question::join('subjects', 'questions.subject_id', '=', 'subjects.id')
                ...
                ->join('difficulty_levels', 'questions.difficulty_level_id', '=', 'difficulty_levels.id')
                ->join('question_types', 'questions.question_type_id', '=', 'question_types.id')
                ->whereIn('questions.question_type_id', [2, 5, 6])
                ->where('questions.is_published', 1)
                ->where('questions.is_approved', 1);
        } else {
            $values = Question::join('subjects', 'questions.subject_id', '=', 'subjects.id')
        ...
        if ($q && $opts) {
            foreach ($opts as $key => $opt) {
                if ($opt == "advanced_id") {
                    $question_id = Advanced_test_question::where('advanced_test_id', $q[$key])->pluck('question_id')->toArray();

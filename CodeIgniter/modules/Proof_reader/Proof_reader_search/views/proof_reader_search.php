            ...
            <div class="box-body">
				<form method="post" class="login-form" action="<?php echo base_url(); ?>index.php/Proof_reader_search" enctype="multipart/form-data">
					<div class="row">
						...
						<div class="col-xs-3">
							<input class="form-control" name="name" id="name" placeholder="Name" type="text" value="<?php echo $this->session->proof_reader_search['name'];?>" required >
						</div>
						<div class="col-xs-3">
							<select class="form-control" id="users" name="users" required="">
								<option value="assignment_id" <?php if($this->session->proof_reader_search['users'] == 'assignment_id'){ echo 'selected'; } ?> >Assignment ID</option>
								<option value="assignment_name" <?php if($this->session->proof_reader_search['users'] == 'assignment_name'){ echo 'selected'; } ?> >Assignment Name</option>
							</select>
						</div>
						<div class="col-xs-2">
				...
				foreach($datas as $assignment)
				{
					...
					{
						$manager_id = $this->Mdl_proof_reader_search->manager_id($assignment['id']);
						$proof_reader_id = $this->Mdl_proof_reader_search->proof_reader_id($assignment['id']);
						$writer_id = $this->Mdl_proof_reader_search->writer_id($assignment['id']);
					?>
						<div class="row">
							<div class="col-xs-3"><br/></div>
							<div class="col-xs-6">
							
								<a href="<?php echo base_url(); ?>index.php/Proof_reader_dashboard/assignment_view/<?php echo $assignment['id'];?>" class="info-box bg-yellow">
				...
				<?php
				if($this->session->proof_reader_search != null){
					if($datas == null){?>
					<div class="row">
						<div class="col-xs-3"><br/></div>
						<div class="col-xs-6">

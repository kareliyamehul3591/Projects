...
class Proof_reader_message extends MX_Controller 
{
	function __construct()
	{
		parent::__construct();
		date_default_timezone_set("Asia/Kolkata");
		$this->load->model('Mdl_proof_reader_message');
	}
	public function index()
	{
		if($this->session->Admindetail['proof_reader'] == 1)
		{
			$user = $this->Mdl_proof_reader_message->user();
			$data=array(
				'user'=>$user,
				'main_content'=>'proof_reader_message',
				'left_sidebar'=>'Proof Reader Message',
			);
	...
	{
		$user = $this->Mdl_proof_reader_message->message_add();
		redirect('Manager_message?id='.$_POST['to_id']);
	}
	public function message_get()
	{
		$this->Mdl_proof_reader_message->message_status($_POST['id']);
		$message = $this->Mdl_proof_reader_message->message($_POST['id']);
		$data = '';	
		$i = 0;
		foreach($message as $messag)
		{
			if($i >= $_POST['count'])
			{
				$user_id = $this->Mdl_proof_reader_message->user_id($messag['from_id']);
	...
	public function header_message_get(){
		$messages = $this->Mdl_proof_reader_message->header_message_get($_POST['to_role']);
		...
		foreach($message as $messag){
			$from_id = $this->Mdl_proof_reader_message->user_id($messag['from_id']);
						...
						<a href="'.$url.'">
							...
							<h4>'.ucfirst($from_id[0]['name']).'<small style="left: 53px">('.ucfirst($messag['from_role']).') (ID '.$messag['assignment_id'].')</small>
								<small><i class="fa fa-clock-o"></i> 
									'.$this->Mdl_proof_reader_message->time_to_hours(strtotime($messag['created_date'])).' ago'.'
								</small>
							</h4>
							<p>'.$messag['message'].'</p>
						</a>
		...
		$data .= '</ul>
			...
			<li class="footer"><a href="'.base_url().'index.php/Proof_reader_message">See All Messages</a></li>
			</ul>';
	...
	{
		$admin_notifications = $this->Mdl_proof_reader_message->header_notifications_get();
			...
			<ul class="dropdown-menu" >
				...
				<li>
					...
					$admin_notifications = array_slice($admin_notifications, 0, 10);
						foreach($admin_notifications as $notifications)
						{
							$from_id = $this->Mdl_proof_reader_message->user_id($notifications['from_id']);
							$assignment_id = $this->Mdl_proof_reader_message->assignment($notifications['assignment_id']);
							$proof_reader_id = $this->Mdl_proof_reader_message->assign_to_ma_id($notifications['assignment_id']);
							...
							$data .='" >';
									if($proof_reader_id == null || $proof_reader_id[0]['status'] == 2)
									{
										...
										$data .='<a href="'.base_url().'index.php/Proof_reader_dashboard/assignment_view/'.$notifications['assignment_id'].'">';
									}
								  $data .=' <h4 style="margin-left: 10px;">
									'.ucfirst($from_id[0]['name']).' <small style="left: 65px">('.ucfirst($notifications['from_role']).')</small>
									<small><i class="fa fa-clock-o"></i> '.$this->Mdl_proof_reader_message->time_to_hours(strtotime($notifications['created_date'])).' ago'.'</small>
								  </h4>';
		...
		$data .= '</ul>
				...
				<li class="footer"><a href="'.base_url().'index.php/Proof_reader_notifications">See All Notifications</a></li>
			</ul>';
		$header_notifications_get_all = $this->Mdl_proof_reader_message->header_notifications_get_all();

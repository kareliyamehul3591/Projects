      ...
      <div class="row">
        <div class="col-xs-12">
          ...
          <div class="box box-info">
			...
			<!-- /.box-header -->
            <div class="box-body">
				<form method="post" class="login-form" action="<?php echo base_url(); ?>index.php/Manager_dashboard/assignment_image_add" id="assignment_add" enctype="multipart/form-data" >
					...
					<div class="row">
						...
						<?php $manager_id = $this->Mdl_manager_dashboard->manager_id($datas[0]['id']); ?>	
						
						<?php if($manager_id[0]['status'] != 1){ ?>
							<script>
						...
						<?php if($manager_id[0]['status'] == 1){ ?>
							...
							<div class="col-xs-6">
								<?php
									$manager_ids = $this->Mdl_manager_dashboard->manager_id($datas[0]['id']);
									if($manager_ids[0]['manager_id'] == $this->session->Admindetail['id'] && $manager_ids[0]['status'] == 1)
									{
										...
										{ 
											if($file != NULL)
											{
												$key++;
												echo $key." :- ";
												echo '<a href="'.base_url().'uploads/Assignment/'.$datas[0]['id'].'/manager/'.$file.'" target="_blank" >'.$file.'</a>';
					...
					<?php
					$manager_id = $this->Mdl_manager_dashboard->manager_id($datas[0]['id']);
					$writer_id = $this->Mdl_manager_dashboard->writer_id($datas[0]['id']);
					$proof_reader_id = $this->Mdl_manager_dashboard->proof_reader_id($datas[0]['id']);
					if($manager_id[0]['status'] == 5 || $datas[0]['status'] == 4 || $manager_id[0]['status'] == 2 || $manager_id[0]['manager_id'] != $this->session->Admindetail['id'])
					{ ?>
					...
					{ 
						$accept_url = "myModal('".base_url()."index.php/Manager_dashboard/accept_taske/".$datas[0]['id']."')";
						$reject_url = "myModal('".base_url()."index.php/Manager_dashboard/reject_taske/".$datas[0]['id']."')";
						?>
						<?php if($datas[0]['status'] != 0){ ?>
						
							<?php if($datas[0]['created_id'] == null){ ?>
								<div class="box-footer">
				  ...
				  <div class="modal-dialog" style="width: 370px;" >
					<div class="modal-content">
					  ...
					  <div class="modal-footer">
						...
						<?php if($manager_id[0]['status'] == 5 || $datas[0]['status'] == 4){?>
							<a href="<?php echo base_url(); ?>index.php/Manager_dashboard/show_archived" type="button" class="btn btn-primary">Leave Page</a>
						<?php }else{ ?>
				...
				<script type="text/javascript">
					...
					function remove_file(ids,id)
					{
						...
						if (images_r == true) {
							$.ajax({
								type:'POST',
								url: '<?php echo base_url(); ?>index.php/Manager_dashboard/assignment_remove_file',
								data:{ id : id , ids : ids },
				...
				<div class="modal-dialog" role="document">
					<div class="modal-content">
						...
						<div class="modal-footer">
							...
							<a href="<?php echo base_url();?>index.php/Manager_dashboard/re_assign_assignment/<?php echo $assignment_final_data[0]['assignment_id'];?>" type="submit" class="btn btn-primary re_assign_assignment_btn" style="display:none;">Re-Assign</a>
						</div>
					</div>
				</div>
			</div>

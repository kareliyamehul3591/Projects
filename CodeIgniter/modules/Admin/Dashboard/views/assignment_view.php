          ...
          <div class="box box-info">
            ...
            <div class="box-body">
				<form method="post" class="login-form" action="<?php echo base_url(); ?>index.php/Dashboard/assignment_image_add" id="assignment_add" enctype="multipart/form-data" >
								...
								<?php
									...
									{ 
										if($file != NULL)
										{
											$key++;
											echo $key." :- ";
											echo '<a href="'.base_url().'uploads/Assignment/'.$datas[0]['id'].'/admin/'.$file.'" target="_blank" >'.$file.'</a>';
											if($datas[0]['status'] != 5){
								...
								<div class="form-group">
									...
									<div class="col-sm-10">
										<?php 
											...
											<?php if($datas[0]['status'] != 5 && $datas[0]['status'] != 4 && $datas[0]['status'] != 6){
												echo '<i class="fa fa-times" onclick="remove_files('.$i.','.$datas[0]['id'].',0)"></i>&nbsp;';											
								...
								<div class="form-group">
									...
									<div class="col-sm-10">
										<?php 
											...
											<?php
											if($datas[0]['status'] != 5 && $datas[0]['status'] != 4 && $datas[0]['status'] != 6){
												echo '<i class="fa fa-times" onclick="remove_files('.$i.','.$datas[0]['id'].',2)"></i>&nbsp;';											
								...
								<div class="form-group">
									...
									<div class="col-sm-10">
										<?php 
											...
											<?php
											if($datas[0]['status'] != 5 && $datas[0]['status'] != 4 && $datas[0]['status'] != 6){
												echo '<i class="fa fa-times" onclick="remove_files('.$i.','.$datas[0]['id'].',3)"></i>&nbsp;';
											}
											echo ',&nbsp;';
											$i++; } } ?>
									</div>
								...
								<div class="form-group">
									...
									<div class="col-sm-10">
										<?php 
											...
											<?php 
											if($datas[0]['status'] != 5 && $datas[0]['status'] != 4 && $datas[0]['status'] != 6){
												echo '<i class="fa fa-times" onclick="remove_files('.$i.','.$datas[0]['id'].',4)"></i>&nbsp;';
											}
											echo ',&nbsp;';
											$i++; } } ?>
									</div>
					...
					<?php 
						$manager_id = $this->Mdl_dashboard->manager_id($datas[0]['id']);
						$proof_reader_id = $this->Mdl_dashboard->proof_reader_id($datas[0]['id']);
						$writer_id = $this->Mdl_dashboard->writer_id($datas[0]['id']);
					...
					<?php
							if($datas[0]['status'] == 0){
								$approval_url = "myModal('".base_url()."index.php/Assignment/approval_taske/".$datas[0]['id']."')";
								$denied_url = "myModal('".base_url()."index.php/Assignment/denied_taske/".$datas[0]['id']."')";
								?>
								
								<div class="box-footer">
									<button type="button" class="btn btn-default" onclick="<?php echo $approval_url; ?>" > <i class="fa fa-check-square fa-lg" style="color: #00a65a;" ></i>&nbsp; Approval</button>
									<button type="button" class="btn btn-default pull-right" onclick="<?php echo $denied_url; ?>" > <i class="fa fa-window-close fa-lg" style="color: #d72752;" ></i>&nbsp; Denied</button>
								</div>
								<script>
							...
							<?php }else{ ?>
								<div class="box-footer">
									...
									<?php if($datas[0]['status'] != 5 && $datas[0]['status'] != 4 && $datas[0]['status'] != 6 && $datas[0]['status'] != 2){ ?>
				  ...
				  <div class="modal-dialog" style="width: 370px;" >
					<div class="modal-content">
					  ...
					  <div class="modal-footer">
						...
						<?php }else{ ?>
							<a href="<?php echo base_url(); ?>index.php/Dashboard/show_archived" type="button" class="btn btn-primary">Leave Page</a>
						<?php }?>
					  </div>
					</div>
				  </div>
				...
				<script type="text/javascript">
					function remove_file(ids,id)
					{
						...
						if (images_r == true) {
							$.ajax({
								type:'POST',
								url: '<?php echo base_url(); ?>index.php/Dashboard/assignment_remove_file',
								data:{ id : id , ids : ids },
					...
					function remove_files(ids,id,role)
					{
						var images_r = confirm("Are you sure want to delete?");
						if (images_r == true) {
							$.ajax({
								type:'POST',
								url: '<?php echo base_url(); ?>index.php/Dashboard/assignment_remove_files',
								data:{ id : id , ids : ids , role : role },
								success:function(data){
									location.reload();
								}
							});

...
class AuthService {
    constructor( model, userModel, accountModel ) {
        this.model = model;
        this.userModel = userModel;
        this.userService = new UserService( userModel );
        this.accountService = new AccountService( accountModel );
        autoBind( this );
    }
    ...
    async register( data ) {
        try {
            ...
            const accountData = await this.accountService.insert( { 'name': data.name } );
            if ( accountData ) {
                finalData.account = accountData.data._id;
                const userData = await this.userService.insert( finalData );
                await this.accountService.update( accountData.data._id, { 'owner': userData.data._id } );
